...
class report_xml(orm.Model):
    ...
    def print_direct(self, cr, uid, report_id, result, format, printer, context=None):
        ...
        if printer:
            ...
            connection.printFile('PDF', file_name, 'LA FACTURA', options=options)
            #connection.printFile('DESPACHO', file_name, file_name, options=options)
            #connection.printFile('Send_To_OneNote_2013:1', file_name, file_name, options=options)
            #connection.printFile('Microsoft_XPS_Document_Writer:2', file_name, file_name, options=options)
            logger = logging.getLogger('base_report_to_printer')
            logger.info("Printing job : '%s'" % file_name)
        return True

    def print_Copias(self, cr, uid, report_id, result, format, printer, context=None):
        fd, file_name = mkstemp()
        try:
            os.write(fd, base64.decodestring(result))
        finally:
        ...
        if printer:
            ...
            options = self.set_print_options(cr, uid, report_id, format, context=context)
            connection.printFile(printer_system_name, file_name, file_name, options=options)
            connection.printFile('PDF', file_name, 'Copias', options=options)
            connection.printFile('Microsoft_XPS_Document_Writer:2', file_name, file_name, options=options)
            #connection.printFile('DESPACHO', file_name, file_name, options=options)
            #connection.printFile('Send_To_OneNote_2013:1', file_name, file_name, options=options)
            #connection.printFile('Microsoft_XPS_Document_Writer:2', file_name, file_name, options=options)
