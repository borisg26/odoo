...
class PrestaShopWebServiceImage(PrestaShopWebServiceDict):
    ...
    def get_image(self, resource, resource_id=None, image_id=None,
        ...
        image_content = base64.b64encode(response[2]) #denis 
        response[1]['content-type'] 

        return {
            'type': response[1]['content-type'] , #  response.headers['content-type'],
...
class GenericAdapter(PrestaShopCRUDAdapter):
    ...
    def write(self, id, attributes=None):
        ...
        api = self.connect()
        attributes['id'] = id
        return api.edit(self._prestashop_model,{self._export_node_name: attributes},id)
